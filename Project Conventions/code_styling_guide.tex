...
This document presents the code styling guidelines used within the ELEN4010 project, including naming of variables and formatting of code. Two style guides are provided: one for JavaScript and one for HTML.
...
Use the \texttt{camelCase} convention for names of variables and functions. All names should start with a lower-case letter. Acronyms should be upper-case.
...
Prefer declaring variables with \texttt{let} over \texttt{var} for better scoping whenever appropriate. 
...
Tabs are preferred over spaces for indentations within the code base.
...
An \texttt{else} statement should go on the same line as the preceding close brace if one is present, otherwise should line up with the \texttt{if} statement.
